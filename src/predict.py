...
class LeafGuardPredictor:
    ...
    def predict(self, input_data, img_np=None):
        """
        ...
        # 7. Model Inference with real-time HSV color checks override
        if img_np is not None:
            ...
            lower_brown_yellow = np.array([8, 30, 20])
            upper_brown_yellow = np.array([33, 255, 255])
            mask_by = cv2.inRange(hsv, lower_brown_yellow, upper_brown_yellow)
            diseased_pixels = np.sum(mask_by > 0)
            
            total_leaf_pixels = green_pixels + diseased_pixels + 1e-6
            disease_ratio = diseased_pixels / total_leaf_pixels
            
            # If more than 4.5% of the leaf area contains yellow/brown spots, classify as Diseased
            if disease_ratio > 0.045:
                status = "Diseased"
                confidence = float(min(0.55 + (disease_ratio * 4.0), 0.98))
            else:
                status = "Healthy"
                green_ratio = green_pixels / total_leaf_pixels
                confidence = float(min(0.55 + (green_ratio * 0.43), 0.98))
        else:
